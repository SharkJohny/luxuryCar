...
function intIndex() {
  ...
  $("button.text-with-icon.group").on("click", function () {
    if (!$("button.text-with-icon.group.less")[0]) {
      $(".feature-chart__table-row[hidden]")
        .removeAttr("hidden")
        .addClass("addHidden");
      ...
      $(".feature-chart__table-row.addHidden")
        .attr("hidden", true)
...
function dinamicPictures() {
  ...
  function changeImage() {
    ...
    sections.each(function () {
      ...
      if (
        sectionTop <= $(window).height() / 2 &&
        sectionBottom >= $(window).height() / 2
      ) {
...
function initModelSelect() {
  ...
  const choiceWrap = $("<div>")
    .addClass("modl-selector-wrap")
    .appendTo(container);
  const znacka =
    `
  ...
  $(".brands select").on("change", function () {
    if ($(this).val() === cstm_znacka.at(1)) {
      ...
      for (let i = 0; i < models_for_brand.length; i++) {
        $("<option>" + models_for_brand.at(i) + "</option>").appendTo(
          ".models select"
        );
      }
    }
...
function initProduct() {
  ...
  buttons.on("click", function () {
    ...
    $(".timeline__slide")
      ...
      .addClass("reveal-invisible")
      .attr("style", "opacity: 0; visibility: hidden; z-index: 0;");
...
function priplatky() {
  if ($(".type-detail").length) {
    ...
    const upsaleText = $("<div>", {
      class: "upsale-text",
      style:
        "background-image: url(https://cdn.myshoptet.com/usr/677818.myshoptet.com/user/documents/upload/assets/new/four.jpg)",
    }).insertAfter(".detail-parameters");
    ...
    $("<div>", { class: "btn button upsale", text: "Koupit výhodně" }).appendTo(
      upsaleText
    );
    ...
    $(".btn.button.upsale").on("click", function () {
      ...
      $("button.btn.btn-lg.btn-conversion.add-to-cart-button").removeClass(
        "upsale"
      );
    });
    $(".add-to-cart").on(
      "click",
      "button.btn.btn-lg.btn-conversion.add-to-cart-button.upsale",
      function (e) {
        e.stopPropagation();
        e.preventDefault();
        createUpsalePopup();
    ...
    $(".button.option-button").on("click", function () {
      ...
      const parameterId = $(this)
        .parents(".parameter-wrap")
        .attr("data-parameterid");
      const image = $(this).find("img").attr("src");
      $(".navigatte-button.parameterNav" + parameterId).attr(
        "style",
    ...
    if ($("html[lang='cs']").length) {
      $(
        ".p-variants-block .surcharge-list:contains('Velikost boxu') option[data-index='0']"
      ).text("Zvolte velikost boxu");
      $(
        ".p-variants-block .surcharge-list:contains('Rozměr 2. Boxu') option[data-index='0']"
      ).text("Zvolte velikost 2.boxu");

      $(
        ".p-variants-block .surcharge-list:contains('Rozměr boxu') option[data-index='0']"
      ).text("Zvolte velikost boxu");
      $(
        ".p-variants-block .surcharge-list:contains('Velikost 2. Boxu') option[data-index='0']"
      ).text("Zvolte velikost 2.boxu");

      $(
        ".p-variants-block .surcharge-list:contains('Barva boxu') option[data-index='0']"
      ).text("Zvolte barvu boxu");
      $(
        ".p-variants-block .surcharge-list:contains('Barva 2. boxu') option[data-index='0']"
      ).text("Zvolte barvu 2.boxu");

      $(
        ".p-variants-block .surcharge-list:contains('Umístění volantu') option[data-index='0']"
      ).text("Prosím, vyberte umístění volantu");
    }
    if ($("html[lang='sk']").length) {
      $(
        ".p-variants-block .surcharge-list:contains('Veľkosť boxu') option[data-index='0']"
      ).text("Zvoľte veľkosť boxu");
      $(
        ".p-variants-block .surcharge-list:contains('Rozmer 2. Boxu') option[data-index='0']"
      ).text("Zvoľte veľkosť 2.boxu");

      $(
        ".p-variants-block .surcharge-list:contains('Rozmer boxu') option[data-index='0']"
      ).text("Zvoľte veľkosť boxu");
      $(
        ".p-variants-block .surcharge-list:contains('Veľkosť 2. Boxu') option[data-index='0']"
      ).text("Zvoľte veľkosť 2.boxu");

      $(
        ".p-variants-block .surcharge-list:contains('Farba boxu') option[data-index='0']"
      ).text("Zvoľte farbu boxu");
      $(
        ".p-variants-block .surcharge-list:contains('Farba 2. boxu') option[data-index='0']"
      ).text("Zvoľte farbu 2.boxu");

      $(
        ".p-variants-block .surcharge-list:contains('Umiestenie volantu') option[data-index='0']"
      ).text("Prosím,vyberte umiestnenie volantu");
    }
...
function createOptions(position, orders) {
  ...
  $(options).each(function () {
    ...
    $("<img>", {
      ...
      src:
        "/user/documents/upload/assets/variants/" +
        parameterId +
        "-" +
        value +
        ".jpg?7",
...
function upsalePage(orders) {
  ...
  const pageWrap = $("<div>", { class: "parameter-wrap upsale " }).appendTo(
    ".content-wrap"
  );
  $("<h5>", {
    text: "Rekapitulace",
  ...
  $("<div>", { class: "btn close", text: "Ukoncit konfigurator" }).appendTo(
    buttonWrao
  );
}
